...
class panel extends view {
  ...
  #get_regions(): void {
    ...
    // Use client measurements instead of bounding rect
    const panel_width = this.element.clientWidth;
    const panel_height = this.element.clientHeight;

    // Get position relative to container's content area
    const container_scroll_width = resizerParent.container.scrollWidth;
    const container_scroll_height = resizerParent.container.scrollHeight;

    // Check if scrollbars are present
    const has_vertical_scroll = this.element.scrollHeight > panel_height;
    const has_horizontal_scroll = this.element.scrollWidth > panel_width;

    // Account for scrollbar width (typically 15-17px)
    const scrollbar_offset = 17;

    // Right edge (compensate for vertical scrollbar)
    const effective_right =
      panel_width - (has_vertical_scroll ? scrollbar_offset : 0);
    if (effective_right < resizerParent.client_width - threshold) {
      this.edges.set("right", new right(this));
    }

    // Bottom edge (compensate for horizontal scrollbar)
    const effective_bottom =
      panel_height - (has_horizontal_scroll ? scrollbar_offset : 0);
    if (effective_bottom < resizerParent.client_height - threshold) {
