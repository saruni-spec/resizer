...
class panel extends view {
    ...
    #get_regions() {
        ...
        const panel_left = rect.left -
            (resizerParent.container_rect.left + resizerParent.client_left);
        const panel_top = rect.top - (resizerParent.container_rect.top + resizerParent.client_top);
...
class edge {
    ...
    #share_border(rect1, rect2, alignment) {
        ...
        // For horizontal edges (top/bottom borders)
            ...
            const horizontal_ovrlap = Math.max(rect1.left, rect2.left) < Math.min(rect1.right, rect2.right);
            return touching && horizontal_ovrlap;
        }
    }
    // Add this method to the edge class
    updatePositionAfterScroll(deltaX, deltaY) {
...
class left extends vertical_edge {
    ...
    resize(rect, change_x, change_y) {
        ...
        console.log(change_x, this.panel.element.id, rect.left, new_left);
    }
}
///
// Right border region
class right extends vertical_edge {
    ...
    resize(rect, change_x, change_y) {
        ...
        console.log(change_x, this.panel.element.id, right_width);
    }
}
