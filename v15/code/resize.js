...
export class resizer extends view {
    ...
    constructor(grid_container, parent, options) {
        ...
        //
        // Also track container scrolling if it's not the body
        if (grid_container && grid_container !== document.body) {
            grid_container.addEventListener("scroll", () => this.update_scroll_position());
        }
        //
    ...
    update_scroll_position() {
        ...
        if (this.container !== document.body) {
            containerScrollX = this.container.scrollLeft;
            containerScrollY = this.container.scrollTop;
            //
        ...
        this.scrollLeft = windowScrollX + containerScrollX;
        this.scrollTop = windowScrollY + containerScrollY;
        //
        // Only notify panels if scroll actually changed
        if (this.scrollLeft !== oldScrollLeft || this.scrollTop !== oldScrollTop) {
            this.panels.forEach((panel) => panel.handle_panel_scroll(this.scrollLeft, this.scrollTop));
        }
    }
    //
...
class panel extends view {
    ...
    constructor(element, parent, options) {
        ...
        this.resizeObserver = new ResizeObserver(() => this.hadnle_resize());
        this.resizeObserver.observe(this.element);
    }
    //
    // Handle scroll events
    handle_panel_scroll(scrollLeft, scrollTop) {
        this.edges.forEach((edge) => edge.adjust_edge_on_scroll(scrollLeft, scrollTop));
    }
    //
    ...
    hadnle_resize() {
        ...
        this.edges.forEach((edge) => {
            ...
            if (edge instanceof right) {
                ...
                edge.element.style.top = `${this.element.clientHeight - top.threshold}px`;
            }
            //
            // Reapply scroll adjustments
            const resizerParent = this.parent;
            edge.adjust_edge_on_scroll(resizerParent.scrollLeft, resizerParent.scrollTop);
        });
    }
    //
...
class edge {
    ...
    adjust_edge_on_scroll(scrollLeft, scrollTop) {
        //
