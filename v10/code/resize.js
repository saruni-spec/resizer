...
export class resizer extends view {
    ...
    constructor(grid_container, parent, options) {
        ...
        this.panels = new Map();
        // If a grid container is provided, use its scrollWidth and scrollHeight
        // instead of just clientWidth and clientHeight
        if (grid_container) {
            this.viewport = [grid_container.scrollWidth, grid_container.scrollHeight];
        }
        else {
            // Use document dimensions including scroll areas
            this.viewport = [
...
class panel extends view {
    ...
    #get_regions(vw, vh) {
        ...
        // Top border
        if (rect.top + scrollY > threshold)
            this.edges.set("top", new top(this));
        // Bottom border
        if (rect.bottom + scrollY < vh - threshold + scrollY)
            this.edges.set("bottom", new bottom(this));
        // Left border
        if (rect.left + scrollX > threshold)
            this.edges.set("left", new left(this));
        // Right border
        if (rect.right + scrollX < vw - threshold + scrollX)
...
class edge {
    ...
    #on_mouse_move(e) {
        ...
        const change_x = e.clientX + scrollX - (this.resize_start[0] + scrollX);
        const change_y = e.clientY + scrollY - (this.resize_start[1] + scrollY);
    ...
    #share_border(rect1, rect2, alignment) {
        ...
        if (alignment === "vertical") {
            ...
            const touching = Math.abs(rect1.right + scrollX - (rect2.left + scrollX)) < closeness ||
                Math.abs(rect2.right + scrollX - (rect1.left + scrollX)) < closeness;
            // Check if they overlap vertically
            const vertical_overlap = Math.max(rect1.top + scrollY, rect2.top + scrollY) <
                Math.min(rect1.bottom + scrollY, rect2.bottom + scrollY);
        ...
        // For horizontal edges (top/bottom borders)
            ...
            const touching = Math.abs(rect1.bottom + scrollY - (rect2.top + scrollY)) < closeness ||
                Math.abs(rect2.bottom + scrollY - (rect1.top + scrollY)) < closeness;
            // Check if they overlap horizontally
            const horizontal_overlap = Math.max(rect1.left + scrollX, rect2.left + scrollX) <
                Math.min(rect1.right + scrollX, rect2.right + scrollX);
