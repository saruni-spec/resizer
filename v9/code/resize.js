...
export class resizer extends view {
    ...
    constructor(parent, options, panel_ids) {
        super(parent, options);
        this.panels = new Map();
        //
        // Get the grid elements on the page
        if (!panel_ids) {
            panel_ids = this.get_panels();
        }
        //
        // Create the panels
        panel_ids.forEach((panel_id) => {
            this.panels.set(panel_id, new panel(panel_id, this));
        });
        //
    ...
    get_panels(gridContainer) {
        ...
        const gridAreas = computedStyle.getPropertyValue("grid-template-areas");
        //
        // Parse the grid areas into an array of area names
        const areaNames = gridAreas
            //
            // Remove quotes
            .replace(/["']/g, "")
            //
            ...
            .trim()
            //
            // Split into an array of area names
            .split(/\s+/);
        // Remove duplicates and empty strings
        const uniqueAreaNames = [...new Set(areaNames)].filter(Boolean);
        //
        // Get the elements associated with each grid area
        uniqueAreaNames.forEach((areaName) => {
            const element = this.get_element(areaName);
            panel_ids.push(element.id);
        });
        return panel_ids;
    }
    //
